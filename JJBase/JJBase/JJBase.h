//
//  JJBase.h
//  JJBase
//
//  Created by xiedong on 2020/9/25.
//  Copyright © 2020 xiedong. All rights reserved.
//

#import <Foundation/Foundation.h>

//! Project version number for JJBase.
FOUNDATION_EXPORT double JJBaseVersionNumber;

//! Project version string for JJBase.
FOUNDATION_EXPORT const unsigned char JJBaseVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <JJBase/PublicHeader.h>
#import <JJBase/JJBaseMacro.h>
#import <JJBase/JJLogMacro.h>

#import <JJBase/NSString+JJCategory.h>
#import <JJBase/NSDate+JJCategory.h>
#import <JJBase/UIColor+JJCategory.h>

#import <JJBase/JJDeviceInfoManager.h>
#import <JJBase/JJLocationInfoManage.h>

#import <JJBase/JJBundleResourceMarco.h>
#import <JJBase/JJBundleResource.h>

#import <JJBase/JJThemeMarco.h>
#import <JJBase/JJThemeManager.h>

#import <JJBase/JJRouter.h>
#import <JJBase/JJLogger.h>
#import <JJBase/JJHandler.h>
#import <JJBase/JJFileUploader.h>
#import <JJBase/JJReachability.h>

#import <JJBase/JJMonitorTime.h>
