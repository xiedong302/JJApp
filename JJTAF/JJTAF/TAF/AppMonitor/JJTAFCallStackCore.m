...
@implementation JJTAFCallStackCore

//为通用回溯设计结构支持栈地址由小到大，地址里存储上个栈指针的地址
typedef struct JJTAFStackFrame {
    const struct JJTAFStackFrame *const previous;
    const uintptr_t return_address;
} JJTAFStackFrame;

//存储 thread 信息的结构体
typedef struct JJTAFThreadInfoStruct {
    double cpuUsage;
    integer_t userTime;
} JJTAFThreadInfoStruct;

static mach_port_t _JJTAFMainThreadId;

+ (void)load {
    _JJTAFMainThreadId = mach_thread_self();
}

//MARK: - Interface
+ (NSString *)callStackWithType:(JJTAFCallStackType)type {
    if (type == JJTAFCallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:JJTAFStackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
        ...
        return [reStr copy];
    } else if (type == JJTAFCallStackTypeMain) {
        //主线程
        NSString *reStr = JJTAFStackOfThread((thread_t)_JJTAFMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_JJTAFMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        NSLog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (pt) {
                ...
                if (!strcmp(name, [nsthread name].UTF8String)) {
                    ...
                    reStr = JJTAFStackOfThread(list[i]);
        ...
        [nsthread setName:originName];
        reStr = JJTAFStackOfThread(mach_thread_self());
        NSLog(@"%@",reStr);
        return [reStr copy];
    }
    return @"";
...
//当前栈帧结构
NSString *JJTAFStackOfThread(thread_t thread) {
    //thread info
    JJTAFThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = JJTAFThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, JJTAFThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = JJTAFMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = JJTAFMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    JJTAFStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = JJTAFMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || JJTAFMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || JJTAFMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    JJTAFSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", JJTAFOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
//MARK: - buidStack
NSString *JJTAFOutputLog(const int entryNum, const uintptr_t address, const Dl_info *const dlInfo) {
...
//MARK: - MachineContext
kern_return_t JJTAFMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}
//MARK: - Symbolicate
void JJTAFSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        JJTAFDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        JJTAFDladdr(JJTAFInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}

bool JJTAFDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = JJTAFDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = JJTAFSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = JJTAFCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t JJTAFDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t JJTAFCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t JJTAFSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = JJTAFCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
    for (uint32_t i = 0; i < machHeader->ncmds; i++) {
        const struct load_command* loadCmd = (struct load_command*)cmdPtr;
        const segmentCommandByCPU* segmentCmd = (segmentCommandByCPU*)cmdPtr;
 ...
 */
uintptr_t JJTAFMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t JJTAFMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t JJTAFInstructionAddressByCPU(const uintptr_t address){
#if defined(__arm64__)
...
mach_msg_type_number_t JJTAFThreadStateCountByCPU() {
#if defined(__arm64__)
...
*/
thread_state_flavor_t JJTAFThreadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t JJTAFMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
