//
//  JJTAFCallTraceCore.c
//  JJTAF
//
//  Created by xiedong on 2020/11/25.
//

#include "JJTAFCallTraceCore.h"
...
static JJTAFCallRecord *_JJTAFCallRecords;
static int _JJTAFRecordNum;
static int _JJTAFRecordAlloc;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_JJTAFCallRecords) {
                _JJTAFRecordAlloc = 1024;
                _JJTAFCallRecords = malloc(sizeof(JJTAFCallRecord) * _JJTAFRecordAlloc);
            }
            _JJTAFRecordNum++;
            if (_JJTAFRecordNum >= _JJTAFRecordAlloc) {
                _JJTAFRecordAlloc += 1024;
                _JJTAFCallRecords = realloc(_JJTAFCallRecords, sizeof(JJTAFCallRecord) * _JJTAFRecordAlloc);
            }
            JJTAFCallRecord *log = &_JJTAFCallRecords[_JJTAFRecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
void JJTAF_callTraceStop() {
    _call_record_enabled = false;
}

void JJTAF_callTraceConfigMinTime(uint64_t us) {
    _min_time_cost = us;
}
void JJTAF_callTraceConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

JJTAFCallRecord * JJTAF_getCallTraceRecords(int *num) {
    if (num) {
        *num = _JJTAFRecordNum;
    }
    return _JJTAFCallRecords;
}

void JJTAF_clearCallTraceRecords() {
    if (_JJTAFCallRecords) {
        free(_JJTAFCallRecords);
        _JJTAFCallRecords = NULL;
    }
    _JJTAFRecordNum = 0;
}
